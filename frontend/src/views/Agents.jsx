...
export default function Agents() {
    ...
    const actionsMemo = useMemo(() => <Export onExport={() => downloadCSV(filteredAgents)} />, []);
        ...
        },
        {
            ...
            cell: (row) => (
                <div className="d-flex flex-wrap gap-2 py-2">
                  <Link className="btn btn-outline-primary bi bi-eye btn-sm" to={`/users/${row.id}/profile`}> profile</Link>
                  <Link className="btn btn-outline-success bi bi-pen btn-sm" to={`/users/${row.id}/profile/edit`}> edit</Link>
                </div>
              ),
          },
    ];
    ...
    const handleSearch = (event) => {
        ...
        const filtered = agents.filter(
          (agent) =>
            ...
            agent.balance.toString().includes(searchTerm) ||
            agent.date_of_birth.includes(searchTerm) ||
